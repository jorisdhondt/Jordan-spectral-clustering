...
function [A, W] = readSparseSingleMatrix (baseFileName)
...
%Z3=fread(fid3,'float');
W= [W single(fread(fid3,'float'))];
fclose(fid3);
disp('Single matrix ingelezen')
%A = spconvert(W);
A = full(W);
[M,N] = size(A);
